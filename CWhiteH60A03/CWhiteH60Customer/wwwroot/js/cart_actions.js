...
function scheduleCartUpdate() {
  ...
  window.cartUpdateTimeout = setTimeout(() => {
    const updatesToSend = Object.entries(cartItemUpdates).filter(
      ([_, update]) =>
        update.quantity !== 0 && Date.now() - update.lastUpdateTime >= 750,
    );
    ...
    fetch(
      "https://api-foilflow.claudewhite.live/api/CartItem/BatchUpdateQuantity",
      {
        method: "PATCH",
        headers: {
      ...
      .catch((error) => {
        ...
        updatesToSend.forEach(([itemId, update]) => {
          ...
          const currentQuantity =
            parseInt(itemQuantity.textContent) - update.quantity;
          itemQuantity.textContent = currentQuantity.toString();

          const price =
            parseFloat(
              itemPrice.textContent.replace("$", "").replace(",", ""),
            ) /
